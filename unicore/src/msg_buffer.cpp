...
#include "unicore/msg_buffer.hpp"
...
void MsgBuffer::GetRawImuMessages(std::vector<unicore::RawImuPtr> &rawimu_massages) {
  rawimu_massages.clear();
  if (!rawimu_msgs_.empty()){
    rawimu_massages.insert(rawimu_massages.end(), rawimu_msgs_.begin(), rawimu_msgs_.end());
    rawimu_msgs_.clear();
  }
}
...
MsgBuffer::ReadResult MsgBuffer::ParseNovatelSentence(
  ...
  if (sentence.id == "RAWIMUSA"){
    unicore::RawImuPtr rawimu = rawimu_parser_.ParseAscii(sentence);
